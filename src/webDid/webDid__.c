...
printf("\nCheck_browser %d\n",browserp->id); 
	tsecs = time((time_t *)0);
	if((tsecs - browserp->last_polled) > 30)
	{
if(WebDID_Debug)
printf("\nDeleting browser %d\n\n",browserp->id); 
		...
		dtq_start_timer(20, check_browser, browserp);
}
...
int update_service_data(char *service, int conn_id, int subscribe, int req, int browser, int force, int formatted)
{
	CURR_SERVICE *servicep;
	time_t tsecs;
