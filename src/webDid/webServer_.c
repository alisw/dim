...
void sendData(int conn_id, char *buffer, int type)
{
	...
	else if(type == 3)
	{
		...
		{
			update_service_data(service, conn_id, 0, req, browser, force, 1);
			return;
		}
	}
	else if(type == 4)
...
if(WebDID_Debug)
	...
	else if(type == 10)
	{
		...
		{
			update_service_data(service, conn_id, 0, req, browser, force, 0);
			return;
		}
	}
	else if(type == 11)
...
printf("Got %s\n", buffer);
			...
			else if(!strncmp(&buffer[5],"dimServiceData",14))
			{
				sendData(conn_id, &buffer[5], 10);
				return;
			}
